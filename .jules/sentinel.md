...
## 2025-02-27 - [Security] Prevent Parameter Injection in Redirect URLs
**Vulnerability:** The application was manually concatenating the `error` parameter directly into a redirect URL string (`new URL('/?error=' + error, request.url)`) within OAuth callback routes (`src/app/api/auth/callback/spotify/route.ts` and `src/app/api/auth/callback/google/route.ts`).
**Learning:** Directly concatenating unvalidated strings into a URL string allows an attacker to inject arbitrary URL query parameters or manipulate the URL structure (HTTP Parameter Pollution/Injection) by supplying an `error` value containing characters like `&` or `=`.
**Prevention:** Always construct URLs dynamically using the `URL` object and manipulate query strings exclusively via `URLSearchParams.set()` (e.g., `url.searchParams.set('error', errorCode)`) to ensure all values are safely and correctly URL-encoded.
