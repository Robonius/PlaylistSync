...
const sha256 = async (plain: string) => {
  if (!window.crypto?.subtle) {
    throw new Error('Web Crypto API (subtle) is not available. This usually happens when the application is not running in a Secure Context (HTTPS or localhost).');
  }
...
export const initiateSpotifyAuth = async (clientId: string, redirectUri: string) => {
  try {
    ...
    console.error('Spotify Auth initiation failed:', error);
    throw error;
  }
};
